...
router.route("/customeraccept").post((req, res) => {
  ...
  Web3Dbb.updateOne(
    { _id: req.body.id },
    { UserStage: req.body.UserStage,Amt:req.body.Amt, HawkerStage:"Success", ContractStage: "Completed" },//changed
    function (err, raw) {
      if (err) {
        res.status(500);
        res.send(err);
...
router.route("/customerdeny").post((req, res) => {
  ...
  Web3Dbb.updateOne(
    { _id: req.body.id },
    { UserStage: "Cancelled", HawkerStage:"Failed", ContractStage: "Completed" },
    function (err, raw) {
      if (err) {
        res.status(500);
        res.send(err);
...
router.route("/hawkerreceive").post((req, res) => {
  ...
  Web3Dbb.updateOne(
    ...
    { ContractStage: "Completed",
      HawkerStage: "Successfully completed"
    },
    function (err, raw) {
      if (err) {
        res.status(500);
...
router.route("/hawkerdeny").post((req, res) => {
  ...
  Web3Dbb.updateOne(
    { _id: req.body.id },
    { HawkerStage: "Cancelled", UserStage:"Hawker Cancellation", ContractStage: "Completed" },
    function (err, raw) {
      if (err) {
        res.status(500);
        res.send(err);
